...
if ($id != "") {
    $querysql = "UPDATE tabel_datasiswa SET nama_siswa = ?, alamat = ?, jurusan = ?, jenis_kelamin = ?, tgl_masuk = ? WHERE tabel_datasiswa.id = ?;";
    $prepare = $conn->prepare($querysql);
    $prepare->bind_param("sssssi", $nama_siswa, $alamat, $jurusan, $kel, $tanggal_masuk, $id);
    $prepare->execute();
    echo $querysql;
} else {
    $querysql = "INSERT INTO `tabel_datasiswa` (`nama_siswa`, `alamat`, `jurusan`, `jenis_kelamin`, `tgl_masuk`) VALUES (?, ?, ?, ?, ?);";
    $prepare = $conn->prepare($querysql);
